...
# Filter simplices corresponding to Delaunay triangulation (lower hull)
simplices = []
for s in ch.vertices:              # s is a list of vertex indices for one facet
    if len(s) != 4:
        # Just in case Qhull ever returns a non-tetrahedral facet,
        # skip it (degenerate case).
        continue
    idx = np.array(s, dtype=int)   # length-4 array of indices

    # Build the 4×4 matrix whose rows are [x_i, y_i, z_i, 1]
    M = np.vstack([X[idx], Y[idx], Z[idx], np.ones(4)]).T

    # Lower hull = det(M) < 0  (orientation test from the slides)
...
def circumscribed_sphere(X: np.ndarray, Y: np.ndarray, Z: np.ndarray, W: np.ndarray):
    """
    Compute the circumscribed sphere of a tetrahedron given by 4 points.

    Parameters
    ----------
    X, Y, Z : np.ndarray of shape (4,)
        Coordinates of the four vertices.
    W : np.ndarray of shape (4,)
        Lifted coordinates (x^2 + y^2 + z^2); not used here, but kept
    ...
    r : float
        Radius of the circumscribed sphere, or -1 if the four points
        are degenerate (co-planar).
    """
    # Build 4x3 array of the tetrahedron vertices
    pts = np.vstack([X, Y, Z]).T.astype(float)  # shape (4, 3)

    # Use point 0 as origin shift
    p0 = pts[0]
    A = pts[1:] - p0          # shape (3, 3); rows are p_i - p0 for i=1..3

    # Right-hand side: |p_i - p0|^2 / 2
    b = np.sum(A ** 2, axis=1) / 2.0

    # If the tetrahedron is degenerate (points coplanar), det(A) ≈ 0
    if abs(np.linalg.det(A)) < 1e-12:
        return np.array([0.0, 0.0, 0.0]), -1.0

    # Solve A * c' = b  for c' = center - p0
    c_prime = np.linalg.solve(A, b)

    center = p0 + c_prime      # actual center in R^3
    r = float(np.linalg.norm(c_prime))
...
for v_idx, pole in poles.items():
    ...
    # vector from point to its pole center
    dir_vec = pole.c - v
    norm = np.linalg.norm(dir_vec)
    if norm > 0.0:
        dir_vec /= norm  # normalize
        Nx[v_idx], Ny[v_idx], Nz[v_idx] = dir_vec
##############################################################
